...
function generateRoundKeys(keyHex) {
  const keyBin = hexToBin64(keyHex.padEnd(16, '0')); 
...
function desEncryptVisualize(plaintextHex, keyHex, rounds = 16) {
  ...
  inputSection.innerHTML = `<h3>Input:<pre>Plaintext: ${plaintextHex}\nKey      : ${keyHex}</pre>`;
  output.appendChild(inputSection);

  const bin = hexToBin64(plaintextHex.padEnd(16, '0')); 
  ...
  for (let i = 1; i <= totalRounds; i++) {
    ...
    const inners = document.createElement('pre');
    inners.innerHTML = `K${i-1}: ${roundKeys[i-1]}\nL${i}: ${L[i]}\nR${i}: ${R[i]}\n`;
    roundOutput.innerHTML += `<h3>Round ${i}:\n`;
    roundOutput.appendChild(inners);
  }

  roundSection.appendChild(roundOutput);

  const preOutput = R[totalRounds] + L[totalRounds]; 
...
function desDecryptVisualize(ciphertextHex, keyHex, rounds = 16) {
  ...
  const asciiop = hexToAscii(hexOutput);
  finalDiv.innerHTML = `<h3>Final decrypted plaintext </h3><pre>Hex: ${hexOutput}\nASCII: ${asciiop}</pre>`;
  output.appendChild(finalDiv);
}
