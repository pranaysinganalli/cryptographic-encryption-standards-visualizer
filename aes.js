...
function keyExpansion(keyHex) {
  ...
  const Nk = 4;  // Number of 4-byte words in key (AES-128)
  const Nb = 4;  // Number of columns (always 4)
  const Nr = 10; // AES-128 has 10 rounds

  const w = [];  // Array of 4-byte words

  // Copy initial key into first 4 words
  for (let i = 0; i < Nk; i++) {
  ...
  return w; // w is an array of 4-byte arrays, total 44 entries
}
...
function aesDecryptVisualize(ciphertextHex, keyHex, rounds = 10) {
  ...
  output.innerHTML += `<pre>Final Plaintext (Hex): ${stateMatrixToHex(state)}</pre>`;
}
